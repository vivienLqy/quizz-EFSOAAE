...
/* Renvoie le tableau de questions/items d'une série, quel que soit son type
   (qcm -> "questions", qcr/trou/schema -> "items"). */
function seriesItems(series, type){
  return type === 'qcm' ? series.questions : series.items;
}

/* Une série n'est pertinente pour la partie actuellement sélectionnée (nav.theme)
   que si au moins une de ses questions/items porte ce tag "module". Sert à ne
   proposer, dans une partie donnée (ex: "Topographie"), que ses propres séries
   et ses propres types d'exercice — jamais ceux des autres parties. */
function seriesMatchesTheme(series, type){
  return seriesItems(series, type).some(x => themeMatch(x.module));
}
...
function renderHome(){
  ...
  MODULES.forEach((mod, i) => {
    ...
    btn.onclick = () => {
      nav = { view: 'module', moduleIndex: i, type: null, seriesIndex: 0, theme: null };
      renderAll();
...
/* ---------------- VUE MODULE ---------------- */
/* pillDefs : liste de { title, index }, où "index" est la position RÉELLE de la
   série dans mod[nav.type] (peut être différente de la position affichée, une
   fois les séries des autres parties filtrées). */
function renderSeriesPills(pillDefs, activeIndex, scoreLabelFn, onSelect){
  const wrap = document.createElement('div');
  wrap.className = 'series-tabbar';
  pillDefs.forEach(({ title, index }) => {
    const pill = document.createElement('button');
    pill.className = 'series-pill' + (index === activeIndex ? ' active' : '');
    const scoreLabel = scoreLabelFn(index);
    pill.innerHTML = `<span>${title}</span>${scoreLabel ? `<span class="pill-score">${scoreLabel}</span>` : ''}`;
    pill.onclick = () => { onSelect(index); };
...
/* Écran affiché juste après avoir choisi une partie (ex: "Armement") : un menu en
   cartes pour choisir le type d'exercice (QCR, Texte à trou, Schéma...), au lieu
   de tomber directement dans un type par défaut. */
function renderTypeChooser(mod, parts){
  const content = document.getElementById('content');
  content.innerHTML = '';
  ...
  const sub = document.createElement('p');
  sub.className = 'quiz-sub';
  sub.textContent = "Choisis le type d'exercice que tu veux réviser.";
  content.appendChild(sub);

  const grid = document.createElement('div');
  grid.className = 'home-grid';
  TYPE_ORDER.forEach(type => {
    // Ne garder que les séries de ce type qui contiennent au moins une question/item
    // de la partie actuellement sélectionnée (nav.theme).
    const list = mod[type].filter(s => seriesMatchesTheme(s, type));
    if(list.length === 0){ return; }
    const card = document.createElement('div');
    card.className = 'module-card';
    const totalItems = list.reduce((acc, s) => acc + seriesItems(s, type).filter(x => themeMatch(x.module)).length, 0);
    card.innerHTML = `<h3>${TYPE_LABELS[type]}</h3>
      <div class="module-stats"><span class="stat">${list.length} série(s) · ${totalItems} question(s)</span></div>`;
    const btn = document.createElement('button');
    btn.className = 'primary';
    btn.textContent = 'Réviser →';
    btn.onclick = () => {
      nav.type = type;
      nav.seriesIndex = 0;
      renderAll();
...
function renderModule(){
  ...
  if(parts.length === 0){
    nav.theme = 'all'; // module sans sous-parties : rien à filtrer par partie
  ...
  const back = document.createElement('button');
  back.className = 'back-btn';
  back.textContent = "← Changer de type d'exercice";
  back.onclick = () => {
    nav.type = null;
    renderAll();
    window.scrollTo({top:0, behavior:'smooth'});
  ...
  // Ne garder que les séries de ce type qui appartiennent à la partie actuellement
  // sélectionnée (nav.theme) : "index" reste la position réelle dans mod[nav.type],
  // utilisée pour indexer l'état (state[...].qcm[index], etc.) et mod.qcm[index].
  const visibleIdx = list.map((s, i) => i).filter(i => seriesMatchesTheme(list[i], nav.type));

  const sub = document.createElement('p');
  sub.className = 'quiz-sub';
  sub.textContent = TYPE_LABELS[nav.type] + (visibleIdx.length ? ' — choisis une série ci-dessous.' : '');
  content.appendChild(sub);
  ...
  if(visibleIdx.length === 0){
    ...
    empty.textContent = `Aucun contenu "${TYPE_LABELS[nav.type]}" pour "${nav.theme}" pour l'instant. Il sera ajouté ici.`;
    content.appendChild(empty);
    return;
  }

  if(!visibleIdx.includes(nav.seriesIndex)) nav.seriesIndex = visibleIdx[0];
  ...
  const pillDefs = visibleIdx.map(i => ({ title: list[i].title, index: i }));
  content.appendChild(renderSeriesPills(pillDefs, nav.seriesIndex, scoreLabelFn, (i) => {
    nav.seriesIndex = i;
    renderAll();
  }));
